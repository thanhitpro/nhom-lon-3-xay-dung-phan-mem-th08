...
namespace EStoreDAO
{
    public class myDongLaptopDAO
    {
        ...
        public List<myDongLaptop> LayDSDongLaptop(int _iMaNhaSX)
        {
            ...
            foreach (CHITIETDONGLAPTOP dongLT in query)
            {
                danhSachDongLaptop.Add(new myDongLaptop() 
                { 
                    IMaDong = dongLT.MaDongLapTop, 
                    STenDong = dongLT.TenChiTietDongLapTop, 
                    SHinhAnh = dongLT.HinhAnh, 
                    SMauSac = dongLT.MauSac, 
                    SMoTaThem = dongLT.MoTaThem, 
                    FGiaBanHienHanh = (float)dongLT.GiaBanHienHanh, 
                    IThoiGianBaoHanh = (int)dongLT.ThoiGianBaoHanh 
                });
            }
            return danhSachDongLaptop;
        }
